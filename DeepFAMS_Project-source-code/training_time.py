...
def training_time(PROJ_DIR, best_snapshots, verbose=False):
    ...
    datasets = [
        str(Path(x).name).replace('_training-runs', '') for x in datasets
    ]
    ...
    for DATASET in datasets:
        ...
        try:
            ...
            parent_dir = str(
                Path(
                    f'{PROJ_DIR}/training_runs/{best_snapshots[DATASET]["file"]}'
                ).parents[1])
        ...
        def find_log_train_time(line):
            ...
            findWholeWord = lambda w, s: re.compile(
            ...
            T = (calc_time(t, 'd') * 24) + calc_time(
                t, 'h') + (calc_time(t, 'm') / 60) + (calc_time(t, 's') / 3600)
            return T
    ...
    table = tabulate(
        [[x, round(d[x]['total_time'], 2), z,
          round(d[x]['FID'], 2)] for (x, y), z in zip(d.items(), days)],
        headers=[
