...
#     fid_logs = {k.replace('FFHQ', 'FFHQ_custom'): v for k, v in fid_logs.items()}
    ...
    files = [
        v.replace('metric-fid50k_full.txt',
                  'log.txt').replace('metric-fid50k.txt', 'log.txt')
        for k, v in fid_logs.items()
    ]
    ...
    md_content = []
    latest_fakes = [
        str(Path(tr + '/' + best_snapshots[k]["file"]).parent) +
        f'/{best_snapshots[k]["snapshot"]}.png'.replace(
            'network-snapshot-', 'fakes') for k, v in best_snapshots.items()
    ]
